...
- generic [ref=e2]:
    ...
    - generic [ref=e54]:
      ...
      - generic [ref=e93]:
        ...
        - textbox "Email address" [ref=e95]:
          ...
          - text: testuser_1783305127804@example.com
      - generic [ref=e96]:
        - generic [ref=e97]: Password
        - generic [ref=e99]:
          - textbox "Password" [ref=e100]:
            - /placeholder: Your password
            - text: "@Cloud#Alpha10_1783305127804"
  ...
  - contentinfo [ref=e123]:
    - generic [ref=e124]:
      ...
      - text: .
    - generic [ref=e129]: v2.3 | Built 2026-07-05 | Angular 20.0.5
...
```ts
  ...
  132 |         if (currentUrl.includes('/auth/register')) {
> 133 |             throw new Error("ระบบไม่ยอมเปลี่ยนหน้า! คุณอาจกรอกข้อมูลไม่ครบหรืออีเมลซ้ำ");
      |                   ^ Error: ระบบไม่ยอมเปลี่ยนหน้า! คุณอาจกรอกข้อมูลไม่ครบหรืออีเมลซ้ำ
  134 |         }
  135 | 
  136 |         await page.locator('input[data-test="email"]').fill(email);
  137 |         await page.locator('input[data-test="password"]').fill(password);
  138 | 
  139 |         
  140 |         
  141 |         
  142 | 
  143 |         // กรณีทำเช็คเรื่อง ค่าที่ไม่ถูก
  144 |         // ลองกรอกค่ามั่วๆ เข้าไป
  145 |         // await dobField.fill('invalid-date');
  146 |         // await dobField.press('Tab');
  147 | 
  148 |         // // ตรวจสอบว่าระบบขึ้น Error จริงไหม (เช็คจาก class 'is-invalid')
  149 |         // await expect(dobField).toHaveClass(/is-invalid/);
  150 | 
  151 |         // // ตรวจสอบข้อความแจ้งเตือน (ถ้ามี element ที่แสดงข้อความ error)
  152 |         // const errorMessage = page.locator('#dob-error'); // อ้างอิงจาก aria-describedby
