...
```
Error: locator.click: Error: strict mode violation: locator('button').filter({ has: locator('[role="img"]') }) resolved to 2 elements:
    1) <button id="language" type="button" aria-expanded="false" data-bs-toggle="dropdown" data-test="language-select" _ngcontent-ng-c2913155081="" aria-label="Select language" class="btn nav-link dropdown-toggle">…</button> aka locator('[data-test="language-select"]')
    2) <button type="button" class="btn btn-outline-secondary">…</button> aka locator('[data-test="login-form"]').getByRole('button').filter({ hasText: /^$/ })

Call log:
  - waiting for locator('button').filter({ has: locator('[role="img"]') })
...
> 32 |         await page.locator('button', { has: page.locator('[role="img"]') }).click();
     |                                                                             ^ Error: locator.click: Error: strict mode violation: locator('button').filter({ has: locator('[role="img"]') }) resolved to 2 elements:
  33 |         const currentValue = await passwordField.inputValue();
  34 |         await page.locator('input[type="password"]').fill(currentValue+'456');
  35 |         await page.locator('button', { has: page.locator('[role="img"]') }).click();
  36 |         //page.keyboard.type('123'); เป็นคำสั่งพิมต่อจากข้อความเดิมเช่น
  37 |         // await page.keyboard.type('123'); // พิมพ์ 123
  38 |         // await page.locator('input[type="button"]').click(); // กดปุ่ม
  39 |         // await page.keyboard.type('456'); // พิมพ์ 456 ต่อเข้าไป (ผลลัพธ์จะเป็น 123456)
  40 | 
  41 | 
  42 |     })
  43 | 
