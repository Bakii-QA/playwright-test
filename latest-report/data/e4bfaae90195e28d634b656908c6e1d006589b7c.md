...
Call log:
  - waiting for locator('input.mat-mdc-input-element').first() to be visible
...
> 28 |                 await inputField.waitFor({ state: 'visible', timeout: 15000 });
     |                                  ^ Error: locator.waitFor: Test timeout of 40000ms exceeded.
  29 |         // 2. ทำ Action เติมข้อความ
  30 |         await inputField.fill("นิช");
  31 |         await page.getByText("เดอะ นิช โมโน รัชวิภา อาคาร บี (ไฟเบอร์) ตึก").waitFor();
  32 |         await page.getByText("เดอะ นิช โมโน รัชวิภา อาคาร บี (ไฟเบอร์) ตึก").click();
  33 |     });
  34 | 
  35 | 
  36 | 
