...
```
Error: locator.click: Error: strict mode violation: getByText(/Temporary login/i) resolved to 2 elements:
...
> 8  |         await page.getByText(/Temporary login/i).click();        
     |                                                  ^ Error: locator.click: Error: strict mode violation: getByText(/Temporary login/i) resolved to 2 elements:
  9  |         await page.getByRole('button', { name: 'Next' }).click();
  10 |         await page.locator("#usernameUserInput").fill("nukornsa");
  11 |         await page.getByTestId('login-page-password-input').fill('myPassword123');
  12 |         await page.getByTestId('login-page-continue-login-button').click();
  13 | 
  14 | 
  15 | 
  16 | 
