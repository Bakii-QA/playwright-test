...
```
TypeError: expect(...).toHaveTitle(...).toBeVisible is not a function
```
...
Call log:
  - Expect "toHaveTitle" with timeout 40000ms
    3 × unexpected value "Just a moment..."
...
```ts
  ...
  142 | 
  143 | 
> 144 |         await expect(page).toHaveTitle("Practice Software Testing - Toolshop - v5.0").toBeVisible();
      |                            ^ Error: expect(page).toHaveTitle(expected) failed
