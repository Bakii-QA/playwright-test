...
- Location: tests/ExTest.spec.js:168:5
...
Call log:
  - waiting for locator('input[data-test="email"]')
...
- generic [ref=e2]:
      ...
      - generic [ref=e183]:
        - generic [ref=e184]:
          - 'link "Combination Pliers Compare Combination Pliers CO₂: A B C D E $14.15" [ref=e185] [cursor=pointer]':
            - /url: /product/01KWX8A04WS7QERAT75JP2D8BC
            - generic [ref=e186]:
          ...
          - 'link "Pliers Compare Pliers CO₂: A B C D E $12.01" [ref=e202] [cursor=pointer]':
            - /url: /product/01KWX8A051PFPHQSN5ESN56326
            - generic [ref=e203]:
              - img "Pliers" [ref=e204]
          ...
          - 'link "Bolt Cutters Compare Bolt Cutters CO₂: A B C D E $48.41" [ref=e219] [cursor=pointer]':
            - /url: /product/01KWX8A054BQZR2Z3GHW9M88QP
            - generic [ref=e220]:
          ...
          - 'link "Long Nose Pliers Compare Long Nose Pliers CO₂: A B C D E Out of stock $14.24" [ref=e236] [cursor=pointer]':
            - /url: /product/01KWX8A057Y0FAC0KF9ZTXT8E1
            - generic [ref=e237]:
          ...
          - 'link "Slip Joint Pliers Compare Slip Joint Pliers CO₂: A B C D E $9.17" [ref=e254] [cursor=pointer]':
            - /url: /product/01KWX8A05AF5TXJ9NGWMWSNAT8
            - generic [ref=e255]:
          ...
          - 'link "Claw Hammer with Shock Reduction Grip Compare Claw Hammer with Shock Reduction Grip CO₂: A B C D E $13.41" [ref=e271] [cursor=pointer]':
            - /url: /product/01KWX8A05DYADX2SRSFMW8KGK1
            - generic [ref=e272]:
          ...
          - 'link "Hammer Compare Hammer CO₂: A B C D E $12.58" [ref=e288] [cursor=pointer]':
            - /url: /product/01KWX8A05KZ3MPVX75RCTPEZXG
            - generic [ref=e289]:
              - img "Hammer" [ref=e290]
          ...
          - 'link "Claw Hammer Compare Claw Hammer CO₂: A B C D E $11.48" [ref=e305] [cursor=pointer]':
            - /url: /product/01KWX8A05RERRRPA2RCTV6T286
            - generic [ref=e306]:
          ...
          - 'link "Thor Hammer Compare Thor Hammer CO₂: A B C D E $11.14" [ref=e322] [cursor=pointer]':
            - /url: /product/01KWX8A05TR7XHW54ZHM2P6WHJ
            - generic [ref=e323]:
  ...
  - contentinfo [ref=e357]:
    - generic [ref=e358]:
      ...
      - text: .
    - generic [ref=e363]: v2.3 | Built 2026-07-06 | Angular 20.0.5
...
```ts
  69  |         //วิธีคลิกปุ่มเมื่อจอเล็ก :1 
  70  |         // await page.mouse.wheel(0, 500);
  71  |         // await page.locator('input[data-test="login-submit"]').click();
  72  | 
  73  |         //วิธีคลิกปุ่มเมื่อจอเล็ก :2
  74  |         let clickCount = 0;
  75  |         const submitBtn = page.locator('input[data-test="login-submit"]');
  76  |         await submitBtn.scrollIntoViewIfNeeded(); 
  77  |         await submitBtn.click();
  78  |         clickCount++; // บวกค่าเพิ่ม 1
  79  |         console.log("มีการคลิกเเล้ว",clickCount +"ครั้ง");
  80  | 
  81  |         const count = await page.locator('input[data-test="login-submit"]').count();
  82  |         console.log("จำนวนปุ่มที่เจอคือ:", count);
  83  | 
  84  |         const a = page.locator('[data-test="login-error"]');
  85  |         await expect(a).toBeVisible();
  86  |         await expect(a).toContainText("Invalid email or password");
  87  | 
  88  |         // ใช้ toHaveText ถ้าต้องการเช็คว่าข้อความตรงกันแบบเป๊ะๆ
  89  |        // await expect(a).toHaveText("Invalid email or password");
  90  |         
  91  |         // หรือใช้ toContainText ถ้าต้องการเช็คแค่ว่ามีข้อความนี้ปรากฏอยู่ (แนะนำตัวนี้มากกว่า เพราะยืดหยุ่นกว่า)
  92  |        // await expect(a).toContainText("Invalid email or password");
  93  | 
  94  |     })
  95  | 
  96  |     test('TC-04 Register your account',async({page})=>{
  97  | 
  98  |         await page.locator('[data-test="nav-sign-in"]').click();
  99  |         const register = await page.locator('a[data-test="register-link"]');
  100 |         await register.scrollIntoViewIfNeeded(); 
  101 |         await register.click();
  102 |         await page.locator('input[data-test="first-name"]').fill("Baaki-QA");
  103 |         await page.locator('input[data-test="last-name"]').fill("Engineer");
  104 | 
  105 |         const dobField = page.locator('[data-test="dob"]');
  106 |         await dobField.fill("1999-11-12");
  107 |         await dobField.press('Tab');
  108 | 
  109 |         const scroll = page.locator('select[data-test="country"]');
  110 |         await scroll.scrollIntoViewIfNeeded(); 
  111 |         await scroll.selectOption("Thailand");
  112 | 
  113 |         const sc = page.locator('input[data-test="phone"]');
  114 |         await sc.scrollIntoViewIfNeeded();
  115 | 
  116 |         await page.locator('input[data-test="postal_code"]').fill("10270");
  117 |         await page.locator('input[data-test="house_number"]').fill("85/54");
  118 |         await expect(page.locator('input[data-test="street"]')).toHaveValue('Abel Fords', { timeout: 5000 });
  119 |         await expect(page.locator('input[data-test="city"]')).toHaveValue('West Melany', { timeout: 5000 });
  120 |         await expect(page.locator('input[data-test="state"]')).toHaveValue('Pennsylvania', { timeout: 5000 });
  121 |         await page.pause();
  122 | 
  123 | 
  124 |         await page.locator('input[data-test="phone"]').fill("0893443321");
  125 |         await page.locator('input[data-test="email"]').fill(email);
  126 |         console.log("ค่าของ email คือ",email);
  127 |         await page.locator('input[data-test="password"]').fill(password);
  128 | 
  129 |         await page.locator('button', { has: page.locator('[data-icon="eye"]') }).click();
  130 |         await page.locator('button[data-test="register-submit"]').click();
  131 | 
  132 | 
  133 | 
  134 |         await page.screenshot({ path: 'debug-after-click.png' }); // แคปไว้ดูว่าหน้าเว็บเป็นไง
  135 | 
  136 |         // 3. ตรวจสอบ URL ปัจจุบัน
  137 |         const currentUrl = page.url();
  138 |         console.log("URL ปัจจุบันคือ:", currentUrl);
  139 | 
  140 |         // 4. ถ้า URL ยังไม่เปลี่ยน แสดงว่า Register ไม่ผ่าน
  141 |         try {
  142 |             await page.waitForURL('https://practicesoftwaretesting.com/auth/login', { timeout: 10000 }); 
  143 |         } catch (error) {
  144 |             // ถ่ายรูปตอนที่รอไม่สำเร็จเพื่อดูหน้าตาของหน้าเว็บหลังจากพยายามสมัครแล้ว
  145 |             await page.screenshot({ path: 'failure-state.png' });
  146 |             throw new Error("ระบบไม่ยอมเปลี่ยนหน้าหลังจากกด Register (อาจเกิดจาก API Error หรือหน้าเว็บค้าง)");
  147 |         }
  148 |         const now = page.url();
  149 | 
  150 |         console.log("URL คือ:", now);
  151 | 
  152 |     //     await page.locator('input[data-test="email"]').fill(email);
  153 |     //    await page.locator('input[data-test="password"]').fill(password);
  154 |     //    await page.locator('button[data-test="login-submit"]').click();
  155 | 
  156 |         // กรณีทำเช็คเรื่อง ค่าที่ไม่ถูก
  157 |         // ลองกรอกค่ามั่วๆ เข้าไป
  158 |         // await dobField.fill('invalid-date');
  159 |         // await dobField.press('Tab');
  160 | 
  161 |         // // ตรวจสอบว่าระบบขึ้น Error จริงไหม (เช็คจาก class 'is-invalid')
  162 |         // await expect(dobField).toHaveClass(/is-invalid/);
  163 | 
  164 |         // // ตรวจสอบข้อความแจ้งเตือน (ถ้ามี element ที่แสดงข้อความ error)
  165 |         // const errorMessage = page.locator('#dob-error'); // อ้างอิงจาก aria-describedby
  166 |         // await expect(errorMessage).toBeVisible();
  167 |     })
  168 |     test('TC05-ลงชื่อเข้าใช้งาน', async ({page})=>{
> 169 |         await page.locator('input[data-test="email"]').fill(email);
      |                                                        ^ Error: locator.fill: Test timeout of 40000ms exceeded.
  170 |         await page.locator('input[data-test="password"]').fill(password);
