...
- Location: tests/ExTest.spec.js:75:5
...
Call log:
  - waiting for locator('a[data-test="register-link"]')
      ...
      - generic [ref=e183]:
        - generic [ref=e184]:
          - 'link "Combination Pliers Compare Combination Pliers CO₂: A B C D E $14.15" [ref=e185] [cursor=pointer]':
            - /url: /product/01KWGFR57N93CC3EY2BSGCVWY1
            - generic [ref=e186]:
          ...
          - 'link "Pliers Compare Pliers CO₂: A B C D E $12.01" [ref=e202] [cursor=pointer]':
            - /url: /product/01KWGFR582W7G5MG0WMA3EPNM9
            - generic [ref=e203]:
              - img "Pliers" [ref=e204]
          ...
          - 'link "Bolt Cutters Compare Bolt Cutters CO₂: A B C D E $48.41" [ref=e219] [cursor=pointer]':
            - /url: /product/01KWGFR585T9RYRQETPGYVSM60
            - generic [ref=e220]:
          ...
          - 'link "Long Nose Pliers Compare Long Nose Pliers CO₂: A B C D E Out of stock $14.24" [ref=e236] [cursor=pointer]':
            - /url: /product/01KWGFR588A197YRY5MM09K719
            - generic [ref=e237]:
          ...
          - 'link "Slip Joint Pliers Compare Slip Joint Pliers CO₂: A B C D E $9.17" [ref=e254] [cursor=pointer]':
            - /url: /product/01KWGFR58AHFZDA6STDVFQ8PRZ
            - generic [ref=e255]:
          ...
          - 'link "Claw Hammer with Shock Reduction Grip Compare Claw Hammer with Shock Reduction Grip CO₂: A B C D E $13.41" [ref=e271] [cursor=pointer]':
            - /url: /product/01KWGFR58DD3K00HKH97NGYDAE
            - generic [ref=e272]:
          ...
          - 'link "Hammer Compare Hammer CO₂: A B C D E $12.58" [ref=e288] [cursor=pointer]':
            - /url: /product/01KWGFR58J6KH3F9Q1T69596XK
            - generic [ref=e289]:
              - img "Hammer" [ref=e290]
          ...
          - 'link "Claw Hammer Compare Claw Hammer CO₂: A B C D E $11.48" [ref=e305] [cursor=pointer]':
            - /url: /product/01KWGFR58N97SY1VHV21RNK7Q7
            - generic [ref=e306]:
          ...
          - 'link "Thor Hammer Compare Thor Hammer CO₂: A B C D E $11.14" [ref=e322] [cursor=pointer]':
            - /url: /product/01KWGFR58S8G7J7QR9E3XR4ED5
            - generic [ref=e323]:
...
```ts
  ...
  30 | 
  31 |         const passwordField = page.locator('input[data-test="password"]');
  32 |         await passwordField.fill('123');
  33 |         await page.locator('button', { has: page.locator('[data-icon="eye"]') }).click();
  34 | 
  35 | 
  36 |         const currentValue = await passwordField.inputValue();
  37 |         await passwordField.fill(currentValue+'45678910');
  38 | 
  39 | 
  40 |         const finalValue = await passwordField.inputValue();
  41 |         // 3. แสดงผลค่าที่ได้
  42 |         console.log("ค่าของ Password คือ", finalValue);
  43 | 
  44 |         //page.keyboard.type('123'); เป็นคำสั่งพิมต่อจากข้อความเดิมเช่น
  45 |         // await page.keyboard.type('123'); // พิมพ์ 123
  46 |         // await page.locator('input[type="button"]').click(); // กดปุ่ม
  47 |         // await page.keyboard.type('456'); // พิมพ์ 456 ต่อเข้าไป (ผลลัพธ์จะเป็น 123456)
  48 | 
  49 |         await page.locator('button', { has: page.locator('[data-icon="eye-slash"]') }).click();
  50 | 
  51 |         //วิธีคลิกปุ่มเมื่อจอเล็ก :1 
  52 |         // await page.mouse.wheel(0, 500);
  53 |         // await page.locator('input[data-test="login-submit"]').click();
  54 | 
  55 |         //วิธีคลิกปุ่มเมื่อจอเล็ก :2
  56 |         const submitBtn = page.locator('input[data-test="login-submit"]');
  57 |         await submitBtn.scrollIntoViewIfNeeded(); 
  58 |         await submitBtn.click();
  59 | 
  60 |         const count = await page.locator('input[data-test="login-submit"]').count();
  61 |         console.log("จำนวนปุ่มที่เจอคือ:", count);
  62 | 
  63 |         const a = await page.locator(".help-block");
  64 |         await expect(a).toBeVisible();
  65 |         await expect(a).toContainText("Invalid email or password");
  66 | 
  67 |         // ใช้ toHaveText ถ้าต้องการเช็คว่าข้อความตรงกันแบบเป๊ะๆ
  68 |        // await expect(a).toHaveText("Invalid email or password");
  69 |         
  70 |         // หรือใช้ toContainText ถ้าต้องการเช็คแค่ว่ามีข้อความนี้ปรากฏอยู่ (แนะนำตัวนี้มากกว่า เพราะยืดหยุ่นกว่า)
  71 |        // await expect(a).toContainText("Invalid email or password");
  72 | 
  73 |     })
  74 | 
  75 |     test('TC-04 Register your account',async({page})=>{
  76 | 
  77 | 
  78 |         const register = await page.locator('a[data-test="register-link"]');
> 79 |         await register.scrollIntoViewIfNeeded(); 
     |                        ^ Error: locator.scrollIntoViewIfNeeded: Test timeout of 40000ms exceeded.
  80 |         await register.click();
  81 | 
  82 | 
  83 | 
  84 | 
